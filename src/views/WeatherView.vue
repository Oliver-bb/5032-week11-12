...
const apikey = "43f418da29b33d5ff15e1f28ec37ad96";
...
export default {
  ...
  computed: {
    ...
    // Here we request metric unit directly from API (no need to convert from Kelvin)
    temperature() {
      return this.weatherData ? Math.round(this.weatherData.main.temp) : null;
    },
    // Get the current weather icon using the API link (use https)
    iconUrl() {
  ...
  methods: {
    ...
    async fetchCurrentLocationWeather() {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ...
          // ← add error handler so the app can continue when geolocation fails
          (err) => {
    ...
    // (Optional) Manual city search to match your Search button in the template
    async searchByCity() {
      if (!this.city) return;
    ...
    async fetchWeatherData(url) {
      try {
        ...
        alert("Failed to retrieve weather information. Please try again later or change the city name.");
      }
    },
  },
};
